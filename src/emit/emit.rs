...
pub fn emit_unit(
    ...
    // Fresh labels in the `.data` segment are generated by
    // string and array literals, and need to be copied.
    if emitter
        .data
        .keys()
        .any(|label| matches!(label, Label::Fresh(_, _)))
    {
...
impl<'env> Emitter<'env> {
    ...
    fn emit_expression(&mut self, expression: &ast::Expression) -> hir::Tree {
        ...
        match expression {
            ...
            Array(expressions, _) => {
                // Note: we don't currently deduplicate array literals like we do strings
                if let Some(mut integers) = expressions
                    .iter()
                    ...
                    .collect::<Option<Vec<_>>>()
                {
                    ...
                    integers.insert(0, Immediate::Integer(length as i64));
                    self.data.insert(label, integers);
