...
class ColoredPoint extends Point {
    ...
    cloneColoredPoint(): ColoredPoint {
        ...
        return createColoredPoint(x, y, r, g, b);
    }

    toString(): int[] {
        x: int, y:int = coords();
        str:int[] = "ColoredPoint at x = " + unparseInt(x) + " and y = " + unparseInt(y) + " and " + col.toString()
        return str
    }
}
...
main(args: int[][]) {
    ...
    cp1: ColoredPoint = createColoredPoint(1000, 2000, 255, 128, 0);
    ...
    p5: Point = p2.add(cp1);
    println(p5.toString());
    println("Should have printed \"Point at x = 1105 and y = 2205\"");
}
